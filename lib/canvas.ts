...
export async function generatePortraitPhotoStrip(photos: string[]): Promise<string> {
	if (photos.length !== 3) {
...
export async function generateLandscapePhotoStrip(photos: string[]): Promise<string> {
	if (photos.length !== 3) {
		throw new Error("Exactly 3 photos are required");
	}
	...
	return new Promise((resolve, reject) => {
		...
		// Landscape dimensions - wider format
		...
		const calculateAndDraw = () => {
			...
			// Calculate photo dimensions - 3 photos side by side
			const availableWidth = canvasWidth - (photoPadding * 2) - (photoGap * 2);
			const photoWidth = Math.floor(availableWidth / 3);

			// Use the first photo's aspect ratio
			const firstPhotoAspect = photoImages[0].height / photoImages[0].width;
			const photoHeight = Math.round(photoWidth * firstPhotoAspect);

			const canvasHeight = headerHeight + photoHeight + (photoPadding * 2) + footerHeight;
			...
			// Draw header with bear logo and text
			const bearLogoSize = headerHeight - 20;
			const bearLogoX = 20;
			const bearLogoY = 10;
			...
			// Draw conference text
			const textX = bearLogoX + bearLogoSize + 20;
			ctx.fillStyle = "#000000";
			ctx.font = "600 24px Jost, sans-serif";
			ctx.textAlign = "left";
			ctx.textBaseline = "middle";
			ctx.fillText("Canadian University Software Engineering Conference", textX, headerHeight / 2);

			// Draw photos side by side
			const photosStartY = headerHeight + photoPadding;
			photoImages.forEach((img, index) => {
				const photoX = photoPadding + (index * (photoWidth + photoGap));

				// Calculate dimensions to maintain aspect ratio
				const imgAspect = img.height / img.width;
				let drawWidth = photoWidth;
				let drawHeight = photoWidth * imgAspect;

				// If taller than our target height, scale down
				if (drawHeight > photoHeight) {
					drawHeight = photoHeight;
					drawWidth = photoHeight / imgAspect;
				}

				// Center the photo in its slot
			...
			let drawFooterHeight = footerHeight - (footerPadding * 2);
			let drawFooterWidth = drawFooterHeight / footerAspectRatio;

			const footerX = (canvasWidth - drawFooterWidth) / 2;
			const footerDrawY = footerY + footerPadding;
...
// Unified export that selects the right generator based on orientation
export async function generatePhotoStrip(photos: string[], orientation: Orientation): Promise<string> {
	if (orientation === "landscape") {
		return generateLandscapePhotoStrip(photos);
	}
	return generatePortraitPhotoStrip(photos);
}
