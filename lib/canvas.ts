export async function generatePhotoStrip(photos: string[]): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    const loadImage = (src: string, onLoad: (img: HTMLImageElement) => void, onError: (error: Error) => void) => {
      const img = new Image();
      img.onload = () => {
        onLoad(img);
        loadedImages++;
    ...
    const calculateAndDraw = () => {
      ...
      // Draw bear logo
      ctx.drawImage(headerImage, bearLogoX, bearLogoY, bearLogoSize, bearLogoSize);
      ...
      for (let split = Math.floor(words.length / 2) - 1; split <= Math.ceil(words.length / 2) + 1; split++) {
      ...
      photoImages.forEach((img, index) => {
        ...
        ctx.drawImage(img, photoPaddingHorizontal, currentY, photoWidth, photoHeight);
      ...
      ctx.drawImage(footerImage, footerX, footerDrawY, drawFooterWidth, drawFooterHeight);
    ...
    photos.forEach((photoData, index) => {
      loadImage(
        photoData,
        (img) => { photoImages[index] = img; },
        (err) => reject(new Error(`Failed to load photo ${index + 1}`))
      );
    });
  });
}
