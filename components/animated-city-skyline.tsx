...
export function AnimatedCitySkyline() {
  const [lights, setLights] = useState<Array<{ x: number; y: number; opacity: number; delay: number }>>([]);
  ...
  useEffect(() => {
    ...
    const newLights = buildings.flatMap((building) => {
      ...
      const numCols = Math.max(1, Math.floor((availableWidth + windowGap) / (windowSize + windowGap)));
      const numRows = Math.max(1, Math.floor((availableHeight + windowGap) / (windowSize + windowGap)));
      ...
      const offsetY = building.yTop + margin + (availableHeight - totalHeight) / 2;
      ...
      for (let row = 0; row < numRows; row++) {
        for (let col = 0; col < numCols; col++) {
          ...
          if (x >= building.x + margin && x <= building.xEnd - margin &&
              y >= building.yTop + margin && y <= building.yBottom - margin) {
            lights.push({
              x,
              y,
              opacity: Math.random() * 0.3 + 0.5,
    ...
    <div className="absolute bottom-0 left-0 right-0 h-[40%] min-h-[250px]">
      ...
      >
        <defs>
          ...
          <pattern id="noise-pattern" x="0" y="0" width="4" height="4" patternUnits="userSpaceOnUse">
          ...
          <linearGradient id="building-shadow" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#0B044D" stopOpacity="1" />
